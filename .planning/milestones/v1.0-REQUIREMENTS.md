# Requirements Archive: v1.0 Stabilization & Optimization
...
- [x] **TEST-05**: TypeScript hook（useTauriCommand, useTauriQuery）のエラーハンドリングテストを作成する
- [x] **TEST-06**: React コンポーネントの部分データレンダリングテストを作成する（null サマリー, 画像ロード失敗）
- [x] **TEST-07**: cargo-llvm-cov と @vitest/coverage-v8 によるカバレッジインフラを導入する
