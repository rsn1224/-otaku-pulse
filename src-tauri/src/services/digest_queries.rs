...
/// ダイジェスト一覧を取得する
pub async fn list_digests(db: &SqlitePool, category: Option<&str>) -> Result<Vec<DigestDto>, AppError> {
    let digests = if let Some(cat) = category {
        sqlx::query("SELECT id, category, title, content_markdown, content_html, 
...
pub async fn insert_digest(db: &SqlitePool, digest: &Digest) -> Result<i64, AppError> {
    let result = sqlx::query("INSERT INTO digests (category, title, content_markdown, content_html, 
                              article_ids, model_used, token_count, generated_at) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
...
/// 未要約の記事を取得する
pub async fn unsummarized_articles(db: &SqlitePool, category: &str, limit: i64) -> Result<Vec<Article>, AppError> {
    // 24時間前のタイムスタンプを計算
    let now = chrono::Utc::now();
    let twenty_four_hours_ago = (now - chrono::Duration::hours(24)).to_rfc3339();
    
    let rows = sqlx::query("SELECT a.id, a.feed_id, a.external_id, a.title, a.url, a.url_normalized,
...
pub async fn get_latest_digest(db: &SqlitePool, category: &str) -> Result<Option<DigestDto>, AppError> {
    let row = sqlx::query("SELECT id, category, title, content_markdown, content_html, 
                          article_ids, model_used, token_count, generated_at 
...
mod tests {
    ...
    async fn setup_test_db() -> SqlitePool {
        ...
        sqlx::query("CREATE TABLE articles (
            id INTEGER PRIMARY KEY,
        ...
        sqlx::query("CREATE TABLE digests (
            id INTEGER PRIMARY KEY,
            category TEXT NOT NULL,
            title TEXT NOT NULL,
    ...
    async fn test_unsummarized_articles() {
        ...
        sqlx::query("INSERT INTO articles (feed_id, title, content, created_at) 
